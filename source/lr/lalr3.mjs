...
import { FIRST, isNonTerm, filloutGrammar, Item, EMPTY_PRODUCTION, types } from "../common2.mjs";
import { gotoCollisionCheck, reduceCollisionCheck, shiftCollisionCheck } from "./error3.mjs";
...
function processState(items, state, states, grammar, items_set, error, LALR_MODE = false) {
    ...
    for (let i = 0, l = items.length; i < l; i++) {
        ...
        if (offset >= len || tok.type == "empty") { //At an accepting state for this input
            ...
            if (item.body == 0 && k == "$")
                state.action.set(k, { name: "ACCEPT", symbol_type:item.follow.type,  size, production: body.production, body: body.id, len });
                ...
                switch (reduceCollisionCheck(grammar, state, item, error)) {
                    case -1:
                        return false;
                    case 0:
                        state.action.set(k, { name: "REDUCE", symbol_type:item.follow.type, state: state.id, size, production: body.production, body: body.id, len });
                        break;
                }
            }
        } else {
                    ...
                    if (!SKIP) {
                        switch (shiftCollisionCheck(grammar, state, new_state, item, size, error)) {
                            case -1:
                                return false;
                            case 0:
                                state.action.set(tok.val, {name: "SHIFT",  symbol_type:tok.type,  state: new_state.id, body: item[0], original_body: body.id, len: size });
                                break;
                        }
                    }
                }

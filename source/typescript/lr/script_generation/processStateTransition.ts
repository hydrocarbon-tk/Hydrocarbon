...
export function processStateTransition(
    ...
    const action = s[1], k = s[0], length = action.size, body = bodies[action.body], funct = [];
    ...
    switch (action.name) {
        ...
        case StateActionEnum.FORK:
            ...
            const fork_ids = action.actions.map(s => convertSymbols([k, s], SYM_LU, types))
                .map(s =>
        ...
        case StateActionEnum.ACCEPT:
            st_fn_id += "a";
            return_value = (StateActionEnum.ACCEPT | (action.state << 3));
        /*intentional*/
        case StateActionEnum.REDUCE:
            if (!return_value) {
                st_fn_id += "r";
                return_value = (StateActionEnum.REDUCE | (length << 3) | (action.production << 11));
            }
            if (length > 0) {
        ...
        case StateActionEnum.SHIFT:
            ...
            if (!return_value) {
                st_fn_id += "s";
                return_value = (StateActionEnum.SHIFT | (action.state << 3));
            }

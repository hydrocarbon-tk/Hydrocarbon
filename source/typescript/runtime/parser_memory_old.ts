...
*/
export function loadWASM(source: BufferSource, memory: WebAssembly.Memory, segments: {
    data_start: number,
    data_end: number,
    debug_start: number,
    debug_end: number,
    ...
    try {
        ...
        function loadBinary(from_buffer: Buffer | Uint8Array, length: number) {
            ...
            const
                { data_start, data_end } = segments,
                max_length = data_end - data_start;

            if (length > data_end - data_start - 1) throw new Error("Buffer too large, max length is " + max_length + " bytes");

            const
                to_buffer = new Uint8Array(memory.buffer, data_start, data_end - data_start),
                len = to_buffer.length;
        ...
        function loadString(str: string) {
            ...
            const
                ...
                buffer = new Uint8Array(memory.buffer, data_start),

                remainder = fillByteBufferWithUTF8FromString(str, buffer, max_length);

            if (remainder < 1) throw new Error("Buffer too large, max length is " + max_length + " bytes");

            buffer[max_length - remainder] = 0;
        };

        function loadStringOld(str: string) {

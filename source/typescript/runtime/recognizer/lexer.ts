...
export function codepoint(reader: ByteReader) {
...
export function byte(reader: ByteReader) {
    return reader.byte();
}
...
function get_utf8_code_point_at(word: number, buffer: Uint8Array): number {

    const a = (word >> 0) & 0x3F;
    const b = (word >> 8) & 0x3F;
    const c = (word >> 16) & 0x3F;
    const d = (word >> 24) & 0x3F;

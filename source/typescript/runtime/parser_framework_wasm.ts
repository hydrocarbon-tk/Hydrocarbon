...
import { RecognizeInitializer } from "../types/parser_framework_types";
import { KernelState, KernelStateBuffer } from './kernel';
...
*/
export async function load_wasm_recognizer(source_path: URI): Promise<RecognizeInitializer> {

    //Detect platform and init URI server if using NodeJS or DENO
    ...
    let memory_buffer: ArrayBuffer = null;
    let memory_buffer_size = 0;
    let memory_buckets = new Map();

    let data_view: DataView = null;
    ...
    function malloc(request_size: number) {
        ...
        let bucket = memory_buckets.get(request_size);
        ...
        if (
            ...
            if (new_ptr > 0)
                (new Uint32Array(memory_buffer, new_ptr - book_keep_size, 1))[0] = request_size;
        }
    ...
    function free(freed_pointer: number) {
        ...
        const [request_size] = (new Uint32Array(memory_buffer, freed_pointer - book_keep_size, 1));
    ...
    function growMemoryIfNeeded(additional_space) {
        if (allocation_head + heap_start + additional_space >= memory_buffer_size - 16) {
            ...
            data_view = new DataView(memory_buffer);
        }
    }
    ...
    const
        module = await WebAssembly.compile(await source_path.fetchBuffer()),
    ...
    class KernelResultsBufferWrapper {
        __ptr__: number;
        constructor(ptr) {
            this.__ptr__ = ptr;
        }
        ...
        get_mut_state(index: number) {
            return <KernelState><any>new KernelStateWrapper(wasm_buffer_get_mut_state(this.__ptr__, index));
        }
    ...
    class LexerWrapper {
        ...
        get byte_offset(): number {
            return data_view.getUint32(this.__ptr__ + 4, true);
        }

        get byte_length(): number {
            return data_view.getUint16(this.__ptr__ + 32, true);
        }
    }

    class KernelIteratorWrapper {
    ...
    return {
        recognize(input_string: string, production_id: number = 0) {
            ...
            return {
                invalid: <KernelStateBuffer><any>new KernelResultsBufferWrapper(invalid_ptr),
                valid: <KernelStateBuffer><any>new KernelResultsBufferWrapper(valid_ptr)
            };
        },

        create_iterator(data: any /* ParserResultsBufferWrapper */) {
            return <any>new KernelIteratorWrapper(wasm_create_iterator(data.__ptr__));
        },
        ...
        init_table() {
            ...
            // This allows the clearing of the heap to be executed as 
            // as trivial index reset when a new parser run is started.

...
const
    ...
    (env, sym, pos) => (env.productions)/*0*/
        ...
        , (env, sym) => new (class { constructor(env, sym, pos) { this.type = "eof"; this.val = "<--eof^^<--eof^^<--eof^^<--eof^^<--eof^^<--eof^^<--eof^^$eof-->-->-->-->-->-->-->"; this.pos = pos; env.symbols.push(this); } })(env, sym)/*57*/
...
export default async function loadParser() {
    ...
    const data = URL.resolveRelative("@candlefw/hydrocarbon/build/wasm/recognizer.wasm").fetchBuffer();

    const wasmModule = await loader.instantiate(data, { env: { memory: shared_memory } }),

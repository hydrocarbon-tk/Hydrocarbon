...
import { GrammarObject, ReverseStateLookupMap } from '../types/index.js';
...
export function renderHTMLStateFromBytecode(
    ...
    const string = [
        ...
        `Address: <b class="address">${address({ c: ((state.pointer & 0xFFFFFF) * 4) })}</b> | Bytesize: ${state.total_size || 0}</div>`,
...
<table class="table-data">
    ...
    <tr><td>DENOMINATOR</td><td>${mod_mask}</td></tr>
                ...
                for (let i = 0; i < table_size; i++) {
                    ...
                    fields.push(ip);
                    const value = getCharacterValueHTML(null, <any>input_type, val);
...
function getInputTypeFromNumber(type: number): "PRODUCTION" | "TOKEN" | "CLASS" | "CODEPOINT" | "BYTE" {
    return ["PRODUCTION", "TOKEN", "CLASS", "CODEPOINT", "BYTE"][(type & 0x7)];
}

...
type KernelStateIterator struct {
	current     *hc_kernel.KernelState
	refs        []*hc_kernel.KernelState
	index       uint32
	final_index uint32
	valid       bool
}

func newKernelStateIterator(state *hc_kernel.KernelState) *KernelStateIterator {
	...
	iter.refs = append(iter.refs[0 : len(iter.refs)-1])

...
namespace HYDROCARBON
{
    ...
    unsigned int getUTF8ByteLengthFromCodePoint(unsigned int code_point)
    {
        if ((code_point) == 0)
        {
            return 1;
    ...
    unsigned int utf8ToCodePoint(unsigned int index, unsigned char *buffer)
    {
    ...
    ParserData::ParserData(unsigned char *input_buffer, unsigned int input_len_in, unsigned int rules_len_in, Lexer *lexer_in)
    {
        lexer = lexer_in;
        state = createState(1);
        prod = 0;
        ...
        origin = 0;
        input = input_buffer;
        rules = new unsigned short[rules_len_in];
        ;
        ;
    }
    void ParserData::sync(ParserData *ptr)
    {
        if ((ptr) == this)
            return;
        ;
    ...
    ParserDataBuffer::ParserDataBuffer()
    {
        len = 0;
        ;
    }
    void ParserDataBuffer::addDataPointer(ParserData *ptr) { data[len++] = ptr; }
    void ParserDataBuffer::removeDataAtIndex(int index)
    {
        len--;
    ...
    DataRef::DataRef(ParserData *ptr_in, bool VALID_in, unsigned int depth_in, unsigned int byte_offset_in, unsigned int byte_length_in, unsigned int line_in)
    {
    ...
    int Lexer ::setToken(int type_in, unsigned int byte_length_in, unsigned int token_length_in)
    {
        type = type_in;
        byte_length = byte_length_in;
    ...
    unsigned int Lexer ::getType(bool USE_UNICODE, ParserData &data)
    {
        unsigned int t = type;
        if (END(data))
            return 1;
        ...
        if ((t) == 0)
        {
            ...
            {
                unsigned int code_point = utf8ToCodePoint(byte_offset, data.input);
    ...
    bool Lexer ::isSym(bool USE_UNICODE, ParserData &data)
    {
        if (((type) == 0 && getType(USE_UNICODE, data) == 2))
        {
            type = 2;
        };
        return (type) == 2;
    ...
    bool Lexer ::isNL()
    {
        if (((type) == 0 && (current_byte) == 10 || (current_byte) == 13))
        {
            type = 7;
        };
        return (type) == 7;
    }
    bool Lexer ::isSP(bool USE_UNICODE, ParserData &data)
    {
        if (((type) == 0 && (current_byte) == 32))
        {
            type = 8;
        };
        return (type) == 8;
    }
    bool Lexer ::isNum(ParserData &data)
    {
        if ((type) == 0)
        {
            if (getType(false, data) == 5)
            {
                unsigned int l = data.input_len;
                unsigned int off = byte_offset;
    ...
    bool Lexer ::isUniID(ParserData &data)
    {
        if (((type) == 0))
        {
            if ((getType(true, data) == 3))
            {
                unsigned int l = data.input_len;
                unsigned int off = byte_offset;
                unsigned int prev_byte_len = byte_length;
                while ((off + byte_length) < l)
                {
                    unsigned int code_point = utf8ToCodePoint(byte_offset + byte_length, data.input);
    ...
    Lexer *Lexer ::copy()
    {
        Lexer *destination = new Lexer();
        Lexer &destination_ref = *destination;
        destination_ref.byte_offset = byte_offset;
        destination_ref.byte_length = byte_length;
        destination_ref.token_length = token_length;
        destination_ref.token_offset = token_offset;
        destination_ref.prev_byte_offset = prev_byte_offset;
        destination_ref.line = line;
        destination_ref.byte_length = byte_length;
        destination_ref.current_byte = current_byte;
        return destination;
    }
    ...
    Lexer &Lexer ::next(ParserData &data)
    {
        byte_offset += byte_length;
        token_offset += token_length;
        if ((data.input_len <= byte_offset))
        {
            type = 1;
            byte_length = 0;
            token_length = 0;
        ...
        {
            current_byte = data.input[byte_offset];
            if ((current_byte) == 10)
                line += 1;
            ;
            type = 0;
    ...
    ParserData *create_parser_data_object(unsigned char *input_buffer, unsigned int input_len, unsigned int rules_len)
    {
        Lexer *lexer = new Lexer();
        ParserData *parser_data = new ParserData(input_buffer, input_len, rules_len, lexer);
        return parser_data;
    }
    ParserData *fork(ParserData &data, ParserDataBuffer &data_buffer)
    {
        ParserData *fork = create_parser_data_object(data.input, data.input_len, data.rules_len - data.rules_ptr);
        (*tip_data).next = fork;
        tip_data = fork;
        ParserData &fork_ref = *fork;
        unsigned int i = 0;
    ...
    void set_action(unsigned int val, ParserData &data)
    {
    ...
    void add_reduce(unsigned int state, ParserData &data, unsigned int sym_len, unsigned int body, bool DNP)
    {
        if (isOutputEnabled(state))
        {
            unsigned int total = body + sym_len;
            if ((total) == 0)
                return;
            ;
            if (body > 0xFF || sym_len > 0x1F)
            {
                unsigned int low = (1 << 2) | (body << 3);
                unsigned int high = sym_len;
                set_action(low, data);
                set_action(high, data);
            }
            else
            {
                unsigned int low = ((sym_len & 0x1F) << 3) | ((body & 0xFF) << 8);
                set_action(low, data);
            };
        };
    }
    void add_shift(ParserData &data, unsigned int tok_len)
    {
        if (tok_len < 0)
            return;
        ...
        if (tok_len > 0x1FFF)
        {
            ...
            unsigned int high = (tok_len & 0xFFFF);
            set_action(low, data);
            set_action(high, data);
        }
        else
        {
            unsigned int low = 1 | ((tok_len << 3) & 0xFFF8);
            set_action(low, data);
        };
    }
    void add_skip(ParserData &data, unsigned int skip_delta)
    {
        if (skip_delta < 1)
            return;
        ;
        ...
        {
            unsigned int low = 2 | (1 << 2) | ((skip_delta >> 13) & 0xFFF8);
            unsigned int high = (skip_delta & 0xFFFF);
            set_action(low, data);
            set_action(high, data);
        }
        else
        {
            unsigned int low = 2 | ((skip_delta << 3) & 0xFFF8);
            set_action(low, data);
        };
    }
    unsigned int mark(unsigned int val, ParserData &data) { return action_ptr; }
    void reset(ParserData &data, Lexer &origin, unsigned int s_ptr, unsigned int r_ptr)
    {
        data.rules_ptr = r_ptr;
        data.stack_ptr = s_ptr;
        (*data.lexer).sync(origin);
    }
    bool consume(Lexer &l, ParserData &data, unsigned int state)
    {
        l.prev_byte_offset = l.byte_offset + l.byte_length;
        if (isOutputEnabled(state))
            add_shift(data, l.token_length);
        ;
        l.next(data);
        return true;
    }
    void pushFN(ParserData &data, StackFunction _fn_ref, int stash)
    {
        data.stack[++data.stack_ptr] = _fn_ref;
        data.stash[data.stack_ptr] = stash;
        data.stash[data.stack_ptr + 1] = stash;
    }
    bool stepKernel(ParserData &data, Lexer &lexer, ParserDataBuffer &data_buffer, int stack_base)
    {
        int ptr = data.stack_ptr;
        StackFunction _fn = data.stack[ptr];
        unsigned int stash = data.stash[ptr];
        data.stack_ptr--;
        int result = _fn(lexer, data, data_buffer, data.state, data.prod, stash);
        data.prod = result;
    ...
    int insertData(ParserData *data, ParserData **resolved, int resolved_len, int resolved_max)
    {
        ParserData &in_ref = *data;
    ...
    int run(ParserData *origin, ParserData **resolved, int resolved_len, int resolved_max, unsigned int base, unsigned int prod_id)
    {
        ;
        ...
        {
            int i = 0;
            for (; i < data_buffer.len; i++)
            {
                ParserData &data = *data_buffer.data[i];
                if ((!stepKernel(data, *data.lexer, data_buffer, base)))
                {
                    data.COMPLETED = true;
                    data.VALID = (data.prod) == prod_id;
                    data_buffer.removeDataAtIndex(i--);
                    resolved_len = insertData(&data, resolved, resolved_len, resolved_max);
                };
            };
        };
        return resolved_len;
    ...
    void clear_data()
    {
        ParserData *curr = root_data;
        if ((curr))
        {
            ParserData *next = (*curr).next;
            while ((curr))
            {
                next = ((*curr).next);
                delete[]((*curr).rules);
    ...
    void init_data(unsigned char *input_buffer, unsigned int input_len, unsigned int rules_len)
    {
        clear_data();
        ParserData *data = create_parser_data_object(input_buffer, input_len, rules_len);
        data_array[0] = data;
    }
    DataRef **get_fork_pointers()
    {
        ...
        {
            ParserData &data = *out_array[i];
            DataRef *fork = new DataRef(out_array[i], (data.VALID), (data.origin_fork + data.rules_ptr), (*data.lexer).byte_offset, (*data.lexer).byte_length, (*data.lexer).line);
            fork_array_len++;
            fork_array[i] = fork;
        };
        return fork_array;
    }
    int block64Consume(ParserData *data, unsigned short *block, unsigned int offset, unsigned int block_offset, unsigned int limit)
    {
    ...
    {
        DataRef &fork_ref = *fork;
        unsigned int remainder = block64Consume(fork_ref.ptr, fork_ref.command_block, fork_ref.command_offset, 0, 64);

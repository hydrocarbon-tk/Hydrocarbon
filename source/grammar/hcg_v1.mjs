...
C0_production=function (sym,env,lex,state) {this.name = sym[1]; this.bodies = sym[3]; this.id = -1; for(let i = 0; i < this.bodies.length; i++){let body = this.bodies[i];for(let j = 0; j < body.sym.length; j++){let sym = body.sym[j];if(sym.optional){let new_sym = body.sym.slice();new_sym.splice(j,1);let new_body = {sym : new_sym, length : new_sym.length, excludes : body.excludes,ignore : body.ignore,error : body.error,functions : body.functions,reduce_function : body.reduce_function};this.bodies.push(new_body);}}}},

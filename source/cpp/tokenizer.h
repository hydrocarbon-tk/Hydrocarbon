...
namespace HC_Tokenizer
{
    ...
    class Token
    {
      ...
      public:
        ...
        friend wostream & operator<<(wostream & os, const Token & tk) { return os << tk.text(); }

        void addSymbol(wstring sym)
        {

            if (!custom_symbols) custom_symbols = (void *) new SymbolMap;
        ...
        wstring text() const { return string.substr(offset, length); }
        ...
        wstring slice(unsigned start = 41923685)
        {
            ...
            if (start == 41923685) start = offset;
        ...
        {
            return string.substr(token.offset,
                                 offset < token.offset ? string_length - token.offset : string_length - offset);
        }
        ...
        Token & next(Token * mk = NULL, bool USE_CUSTOM_SYMBOLS = true)
        {
            ...
            unsigned length = marker.length, offset = marker.offset + length, line = marker.line, base = offset,
                     chr = marker.chr, root = marker.offset;
            ...
            if (USE_CUSTOM_SYMBOLS && marker.custom_symbols != NULL) {
                ...
                while (code == 32 && IGNORE_WHITE_SPACE && offset < string_length) {
                    code = string[++offset2];
                    offset++;
                }

                if (offset >= string_length) goto end;
            ...
            while (NORMAL_PARSE) {
                ...
                if (code < 128) {
                    ...
                    case JUMP_TYPE::NUMBER:
                        while (++offset < string_length && ((code = string[offset]) < 128) &&
                               (12 & number_and_identifier_table[code]))
                            ;
                        type = TYPE::NUMBER;

                        if ((code == L'e' || code == L'E') && number_and_identifier_table[code]) {
                    ...
                    case JUMP_TYPE::STRING:
                        if (!marker.PARSE_STRING) {
                            while (++offset < string_length && (code != string[offset]))
                                ;
                    ...
                    case JUMP_TYPE::SPACE:
                        ...
                        break;
                    case JUMP_TYPE::TAB:
                        while (++offset < string_length && (L'\t' != string[offset]))
                            ;
                        type   = TYPE::WHITE_SPACE;
                        length = offset - base;
                        break;
                ...
                if (IGNORE_WHITE_SPACE &&
                    ((unsigned) type & ((unsigned) TYPE::WHITE_SPACE | (unsigned) TYPE::NEW_LINE))) {

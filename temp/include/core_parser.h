...
class ParserData{public:Lexer* lexer;
    unsigned int state;
    unsigned int prod = 0;
    int stack_ptr = 0
;
...
;
    bool  VALID = 0;
    bool  COMPLETED = 0;
    ParserData (unsigned int , unsigned int , Lexer* );
    void sync(ParserData* );};
class ParserDataBuffer{public:ParserData** data=new ParserData*[64];
    int len ;
    ParserDataBuffer ();
    void addDataPointer(ParserData* );
    void removeDataAtIndex(int );};
class DataRef{public:ParserData* ptr = 0
;
    bool  VALID = 0
;
...
class Lexer {public:unsigned int byte_offset = 0
;
...
ParserData* create_parser_data_object(unsigned int , unsigned int );
...
void  clear_data();
unsigned char* init_data(unsigned int , unsigned int );

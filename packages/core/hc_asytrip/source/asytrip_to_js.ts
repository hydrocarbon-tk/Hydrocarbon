 ...
 * disclaimer notice.
 */
import { ASYTRIPContext, ASYTRIPType, ASYTRIPTypeObj, GrammarObject } from '@hc/common';
import {
...
type HCObjIterator = (node:HCNode<Type>, par:HCNode<Type>, replace_prop_index:number, replace_array_index:number)=>void;
class HCNode<T> {
    constructor(type: T, tok: Token) {
        this.type = type;
        this.tok = tok;
    }
    iterate(iter:HCObjIterator, parent:HCNode<Type>,replace_prop_index:number, replace_array_index:number){}
    replace(node: HCNode<Type>, node_index:number, array_index:number ){}
    type:T;
    tok:Token;
}
...
export class ${name} extends HCNode {
    ...
    iterate(iter, parent, replace_prop_index, replace_array_index){
            ...
            if (HAVE_STRUCT) {
                ...
                if (HAS_STRUCTS) {
                    ...
                    let val = [
                        ...
                        HAVE_OTHERS ? `   if (this.${n}[i] instanceof HCNode)` : "",
...
addExpressMap(ASYTRIPType.EXPRESSIONS, (v, c, inits) => {
    return v.expressions.map(t => typeToExpression(t, c, inits)).join(", ");
});

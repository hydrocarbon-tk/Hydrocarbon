...
export function processStateTransition(s, env, bodies, state_map, state_functions_map, state_str_functions, COMPILE_FUNCTION, fork_map, SYM_LU, types) {
    ...
    switch (state.name) {
        case StateActionEnum.FORK:
            const fork_ids = state.actions.map(s => convertSymbols([k, s], SYM_LU, types))
                .map(s => processStateTransition(s, env, bodies, [], state_functions_map, state_str_functions, COMPILE_FUNCTION, [], SYM_LU, types));

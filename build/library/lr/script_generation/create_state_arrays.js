import { ERROR, ACCEPT, SHIFT, REDUCE, FORK_ACTION, DO_NOTHING, IGNORE } from "../../types/state_action_enums.js";
...
function processStateTransition(s, env, st, bodies, state_map, state_functions_map, state_str_functions, COMPILE_FUNCTION, fork_map, SYM_LU, types) {
    ...
    switch (state.name) {
        case FORK_ACTION:
            var fork_ids = state.actions.map(s => convertSymbols([k, s], SYM_LU, types))
                .map(s => processStateTransition(s, env, { id: 0, pos: Infinity }, bodies, [], state_functions_map, state_str_functions, COMPILE_FUNCTION, SYM_LU, types));
...
export default function (grammar, states, env, functions, SYM_LU, types) {
    const bodies = grammar.bodies, state_functions = [], goto_map_lookup = [], state_str_functions = [], state_functions_map = new Map(), state_maps = [], state_maps_map = new Map(), goto_maps = new Map(), COMPILE_FUNCTION = (env.options) ? !!env.options.integrate : !1, error_handlers = [], fork_map = [];
    for (let i = 0; i < states.length; i++) {
        ...
        [...state.actions.entries()]
            ...
            .reduce((r, s) => (processStateTransition(s, env, r, bodies, state_map, state_functions_map, state_str_functions, COMPILE_FUNCTION, fork_map, SYM_LU, types), r), { id: 0, pos: -1 }); //*/
        ...
        const out_state_map = fillAndCompress(state_map), sm_id = `[${out_state_map.join(",")}]`;

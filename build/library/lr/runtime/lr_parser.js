...
function parser(lex, data = null, e = {}, sp = 1, len = 0, off = 0, o = [], state_stack = [lex.copy(), 0], FORKED_ENTRY = false, state_action_lu = 0, cycles = 0, fork_depth = 0, forks = 0) {
    if (!data)
        return { value: [], error: "Data object is empty" };
    //Unpack parser objects
    const { gt: goto, sym: symbols, lu: token_lu, sts: states, sa: state_action_tables, fns: state_action_functions, eh: error_handlers, gtk: getToken, ty: types, fm: fork_maps } = data, { sym: lex_sym, num: lex_num, ws: lex_ws, nl: lex_nl } = lex.types, { keyword, any, $eof: eof } = types;
    let RECOVERING = 100, FINAL_RECOVERY = false, tk = 0, total_cycles = cycles, state = null, state_length = 0;
    if (!FORKED_ENTRY) {
        lex.IWS = false;
            ...
            if (state_action_lu == 0) {
                ...
                switch (action & 7) {
                    ...
                    case StateActionEnum.FORK:
                        ...
                        var fork_states_start = action >> 16, fork_states_length = (action >> 3) & 0x1FFF, fork_states_end = fork_states_start + fork_states_length, result = { value: null, error: "", cycles, total_cycles, off };
                        FORKED_ENTRY = true;
                        for (const forked_state_action_lu of fork_maps.slice(fork_states_start, fork_states_end)) {
                            const copied_lex = lex.copy(), copied_output = o.slice(), copied_state_stack = state_stack.slice(), res = parser(copied_lex, data, e, sp, len, off, copied_output, copied_state_stack, FORKED_ENTRY, forked_state_action_lu, cycles, sp, forks + 1);
...
*/
export default (lex, data, environment) => {
    const res = parser((typeof lex == "string") ? new Lexer(lex) : lex, data, environment);

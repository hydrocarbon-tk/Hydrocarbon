...
export function LRParserCompilerCPP(states, grammar, env) {
    ...
    //parse body function
    let symbol_index = 1;
    const COMPILE_FUNCTION = (env.options) ? !!env.options.integrate : false, functions = [], error_handlers = [], SYMBOL_INDEX_OFFSET = 15, //Must leave room for symbol types indices
    ...
    const output = Compiler(goto_maps, state_maps, state_functions, SYM_LU, default_error, error_handlers, functions, state_str_functions, goto_functions, GEN_SYM_LU, [...(grammar.meta.symbols || new Map).values()].map(e => e.val));
    return output;
}

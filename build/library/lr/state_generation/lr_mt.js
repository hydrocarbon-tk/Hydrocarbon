...
export class LRMultiThreadRunner {
    ...
    async mergeWorkerData(wkr, to_process_items, state, errors) {
        this.item_set.push(...(to_process_items.filter(i => {
            const id = i.state_id.id, sym = i.state_id.sym;
            if (i.excludes.length > 0)
                console.log(i.excludes);
            i.items = i.items.map(i => Item.fromArray(i));
            if (this.processed_states.has(id)) {
                const set = this.processed_states.get(id), out_items = i.items.filter(i => !set.has(i.full_id) && (set.add(i.full_id), true));

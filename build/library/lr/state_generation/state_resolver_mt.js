...
export default class StateResolver {
    ...
    handleReduceCollision(grammar, state, existing_reduce, new_reduce, errors) {
        ...
        const body_a = new_reduce.item.body_(grammar), body_b = existing_reduce.item.body_(grammar);
    ...
    handleForkShiftCollision(grammar, state, fork_action, reduce_action) {
        //DO Nothing in LALR
    }
    ...
    */
    complete(states_map, grammar) {
        ...
        for (const state of states) {
            ...
            grammar.meta.ignore.forEach(i => i.symbols.forEach((sym) => {
            ...
            grammar.meta.error.forEach(i => i.symbols.forEach((sym) => {
                if (!state.actions.has(sym.val) && state.grammar_stamp == i.grammar_stamp) {

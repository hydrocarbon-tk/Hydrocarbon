const EXCLUDE_MAP = 1, IGNORE_MAP = 2, ERROR_MAP = 3, RESET_MAP = 4, REDUCE_MAP = 5, extractable_symbol_lookup = {
    exc: EXCLUDE_MAP,
    ign: IGNORE_MAP,
    err: ERROR_MAP,
    rst: RESET_MAP,
...
export default function (production, env, lex) {
    ...
    if (production.IMPORT_APPEND || production.IMPORT_OVERRIDE) {
        const imported = production.name;
        ...
        if (imported.RESOLVED)
            imported.resolveFunction(production.name.production);
        return;
    }
    ...
    for (let i = 0; i < bodies.length; i++) {
        ...
        outer: for (let j = 0; j < body.sym.length; j++) {

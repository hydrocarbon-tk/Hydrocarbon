...
function constructCompilerEnvironment(grammar_file_url, global_pending_files, local_pending_files, unique_grammar_file_id, meta_imported_productions) {
    return {
        SLEEP: sleep,
        bodies: new Map(),
        grammarParser,
...
export async function grammarParser(grammar_string, grammar_file_url, unique_grammar_file_id = 112, meta_imported_productions = new Map, global_pending_files = { count: 0 }) {
    const local_pending_files = { count: 0 }, result = parser(whind(grammar_string), hcg_parser_data, constructCompilerEnvironment(grammar_file_url, global_pending_files, local_pending_files, unique_grammar_file_id, meta_imported_productions)), productions = result.value;
    if (result.error) {
        throw result.error;
    }

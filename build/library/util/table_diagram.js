...
import { filloutGrammar } from "./common.js";
export function renderTable(states, grammar, tab = "   ", tab_s = tab.length) {
    ...
    const ws = (count) => (" ").repeat(count), tnts = (h) => ((Math.ceil((h + 0.5) / tab_s) * tab_s) - h), stnts = (h) => ws(tnts(h)), rule_table = states;
    if (rule_table.INVALID) {
        console.error("Invalid state set passed. Cannot continue.");
        return;
    }
    const num_states = rule_table.length, symbols = new Map(), prods = new Map(), preps = [];
    let max_body = 0, symbol_count = 0, prod_count = 0, str = "";
    /****** Preperation ********/
    for (let i = 0; i < num_states; i++) {
        const state = rule_table[i], prep = { action: [], goto: [] };
    ...
    for (let i = 0; i < num_states; i++) {
        ...
        const s = rule_table[i].b.join(" "), diff = max_body - s.length;

...
export function processClosure(items, grammar, error, excludes, offset = 0, added = new Set()) {
    ...
    for (let i = offset; i < g; i++) {
        const item = items[i], body = bodies[item.body], len = item.len, index = item.offset, B = body.sym[index], Be = body.sym.slice(index + 1), b = item.follow, end = items.length, step_excludes = excludes.slice(), new_excludes = [], out_excludes = [];
        ...
        for (let u = 0; u < step_excludes.length; u++) {
            ...
            if (item.body == ex.body && index == ex.inner_offset) {
                const d = ex.offset, j = index, //, clock = 0;
                i_sym = body.sym[j], e_sym = ex.symbols[d];
        ...
        if (index < len && B.type == "production") {
            ...
            for (let i = 0; i < production.bodies.length; i++) {
                const pbody = production.bodies[i], body_index = pbody.id, first_mod = [...first.slice(), ...pbody.reduce];
                ...
                for (let i = 0, l = first_mod.length; i < l; i++) {
                    if (!first_mod[i])
                        continue;
                    const item = new Item(pbody.id, pbody.length, 0, first_mod[i]), sig = item.full_id;
                    if (!added.has(sig)) {
                        items.push(item);
                        added.add(sig);
                    }
                ...
                if (pbody.reduce && pbody.reduce.size > 0)
                    pbody.reduce.forEach(v => v.forEach(s => {
                        const item = new Item(pbody.id, pbody.length, 0, s), sig = item.full_id;
                        if (!added.has(sig)) {
                            items.push(item);
                            added.add(sig);
                        }
            ...
            const added_count = items.length - end, count = processClosure(items, grammar, error, out_excludes, g, added);
